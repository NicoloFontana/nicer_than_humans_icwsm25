...
for timestamp in runs.keys():
    ...
    mean_main_histories = [sum(main_history[window_size:]) / len(main_history[window_size:]) for main_history in main_histories]
...
plt.legend()
plt.tight_layout()
save_plot(plt_fig, OUT_BASE_PATH / "cooperation_bias-window_size")
plt.show()
