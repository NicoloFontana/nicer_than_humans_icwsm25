...
import sys
import time
from src.strategies.strategy_utils import main_blind_strategies, main_hard_coded_strategies, get_strategies, compute_behavioral_profile, get_strategy_by_name
...
n_iterations = 100
checkpoint = 100
main_strategies = get_strategy_by_name("unfair_random")
...
for strategy_name in main_strategies.keys():
    ...
    for defect_prob in range(6, 10):
        ...
        for i in range(2):  # 0: against RND, 1: against self
            ...
            behavioral_profile = compute_behavioral_profile(timestamp, game_histories, main_behavioral_features, main_player_name=main_name, opponent_name=opponent_name)
