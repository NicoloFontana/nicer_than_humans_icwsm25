...
def plot_checkers_results(checkers_names, timestamp):
    results_file_path = merge_checkers_results(checkers_names, timestamp)
    ...
    ax.set_title(f'LLM checks - {timestamp}')
    ...
    plt.tight_layout()
    plt.show()
    plt.savefig(f'{CHECKS_OUT_BASE_PATH}{timestamp}.png')


def merge_checkers_results(checkers_names, timestamp):
    python_objects = {}

    for checker in checkers_names:
        with open(f"{CHECKS_OUT_BASE_PATH}{checker}\\{timestamp}.json", "r") as f:
            python_object = json.load(f)
            for key in python_object.keys():
                python_objects[key] = python_object[key]

    # Dump all the Python objects into a single JSON file.
    out_file_path = f"{CHECKS_OUT_BASE_PATH}{timestamp}.json"
