...
confidence = 0.95
csv_dir_path = Path("../../csv_files_for_plots") / "coop-rnd_round"
csv_dir_path.mkdir(parents=True, exist_ok=True)

no_wdw_game_histories = extract_histories_from_files_(OUT_BASE_PATH / no_wdw_extraction_timestamp / "game_histories", "game_history")
...
wdw_game_histories = extract_histories_from_files_(Path("../../behavioral_profiles_analysis") / "llama2" / "URND05" / "game_histories", "game_history")
...
for iteration_idx in range(len(no_wdw_main_histories[0])):
    ...
    df.to_csv(csv_dir_path / f"llama2_vs_RND_wdw-no_wdw.csv")
